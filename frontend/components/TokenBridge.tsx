...
export function TokenBridge() {
  ...
  const availableChains = CHAIN_OPTIONS.filter(chain => chain.value !== chainId)
  ...
  const handleBridge = async () => {
    ...
    try {
      await bridgeToken(tokenType, amount, dstChain, isONFT ? parseInt(tokenId) : undefined)
      setAmount('')
      setTokenId('')
    } catch (error) {
  ...
  if (!address) {
    return (
      <div className="rounded-2xl bg-gray-800/50 border border-gray-700 p-8 text-center">
        <p className="text-gray-300">Please connect your wallet to use the bridge</p>
      </div>
    )
  }
    ...
    <div className="rounded-2xl bg-gradient-to-br from-blue-500/10 to-cyan-500/10 border border-blue-500/30 p-8">
      ...
      <div className="mb-6 p-4 rounded-lg bg-gray-800/50">
        ...
        <p className="text-xl font-bold">
          {CHAIN_OPTIONS.find(c => c.value === chainId)?.label || 'Unknown Chain'}
        </p>
      </div>
      ...
      <div className="mb-6">
        ...
        <input
          ...
          onChange={(e) => isONFT ? setTokenId(e.target.value) : setAmount(e.target.value)}
          className="w-full px-4 py-3 rounded-lg bg-gray-900 border border-gray-700 focus:border-blue-500 focus:outline-none"
      ...
      <div className="mt-6 p-4 rounded-lg bg-gray-800/30 text-sm text-gray-400">
        <p className="mb-2"><strong>Bridge Information:</strong></p>
